...
export class Permission implements IPermission {
  ...
  public hasActionPermission(
    ...
    const { actions, extraActions } = this.getPolicy(this.policies, route, roleTypes) ?? {
      actions: undefined,
    };
  ...
  private getPolicy(
    ...
    names.slice(sliceIndex).some((name, index) => {
      ...
      if (
        Object.keys(policy.rolePolicy).some((roleType) => roleTypes.includes(<RoleEnum>roleType))
      ) {
